...
describe("FeeSubsityDrip", function () {
  ...
  beforeEach(async function () {
    ...
    const FeeSubsidyDrip = await ethers.getContractFactory(
      "FeeSubsidyDrip",
      dripTokenPool,
      wethPool
    );
    drip = await FeeSubsidyDrip.deploy(
      weth.address,
      dripToken.address,
      router.address,
  ...
  describe("drip", async function () {
    it("drips some token and converts it to ETH", async function () {
      ...
      await drip.drip();
      expect(await ethers.provider.getBalance(drip.address)).to.eq("80");
    });
  });

  describe("buy", async function () {
    it.only("refunds the sender ETH required for a trade", async function () {
      await setup({
        [await alice.getAddress()]: [
          [parseUnits("100"), usd],
      ...
      const tx = await router
        .connect(bob)
        .buyWithSubsity(applePool.address, parseUnits("100"), drip.address);
